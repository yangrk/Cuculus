...
int main( )
{
	...
	//--- init log ----
	string log_file = ini.GetString("DAYLOG","LOG_FILE","../Log/Day");
	int log_level = ini.GetInteger("DAYLOG","LOG_LEVEL",9);
	int log_type = ini.GetInteger("DAYLOG","LOG_TYPE",1);
	...
	LogManager::Open("DayWork", log_type);
	LogManager::SetLogLevel(log_level);

	Logger log;
	log.Info("service DayWorkTask is running!");
