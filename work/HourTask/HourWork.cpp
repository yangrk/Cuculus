...
int main( )
{
	...
	//--- init log ----
	string log_file = ini.GetString("HOURLOG","LOG_FILE","../Log/Hour");
	int log_level = ini.GetInteger("HOURLOG","LOG_LEVEL",9);
	int log_type = ini.GetInteger("HOURLOG","LOG_TYPE",1);
	...
	LogManager::Open("HourWork", log_type);
	LogManager::SetLogLevel(log_level);

	Logger log;
	log.Info("service HourWorkTask is running!");
