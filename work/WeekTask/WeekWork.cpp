...
int main( )
{
	...
	//--- init log ----
	string log_file = ini.GetString("WEEKLOG","LOG_FILE","../Log/Week");
	int log_level = ini.GetInteger("WEEKLOG","LOG_LEVEL",9);
	int log_type = ini.GetInteger("WEEKLOG","LOG_TYPE",1);
	...
	LogManager::Open("WeekWork", log_type);
	LogManager::SetLogLevel(log_level);

	Logger log;
	log.Info("service WeekWorkTask is running!");
